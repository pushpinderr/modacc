...
int main(int argc, char *argv[]) {
    ...
    std::vector<unsigned long> start_times, end_times, start_times_read, end_times_read;

    std::vector<queue> copy_engines;
    auto _devices = device::get_devices(info::device_type::cpu);
    ...
    float *_device_data = (float *)malloc_device(sizeof(float) * num_elements,
                                                 _devices[0], _context);
    int num_elements_read=8192*3072;
    float *_host_data_read = (float *)malloc(sizeof(float) * num_elements_read);
    float *_device_data_read = (float *)malloc_device(sizeof(float) * num_elements_read,
                                                 _devices[0], _context);


    event ev[num_queues];
    event ev_read[num_queues];
    int num_sub_elements = num_elements / num_queues;
    int num_sub_elements_read = num_elements_read / num_queues;
    int subSize = sizeof(float) * num_elements / num_queues;
    int subSize_read = sizeof(float) * num_elements_read / num_queues;
    for (int i = 0; i < num_queues; i++){
        ev[i] =
            copy_engines[i].memcpy(_device_data + i*num_sub_elements, _host_data + i*num_sub_elements, subSize);
        ev_read[i] =
            copy_engines[i].memcpy(_host_data + i*num_sub_elements_read, _device_data + i*num_sub_elements_read, subSize_read);
    ...
    for (int i = 0; i < num_queues; i++)
    {
        ...
        start_times_read.push_back(ev_read[i].get_profiling_info<sycl::info::event_profiling::command_start>());
        end_times_read.push_back(ev_read[i].get_profiling_info<sycl::info::event_profiling::command_end>());
    ...
    for (int i = 0; i < num_queues; i++)
    {
        ...
        std::cout<<start_times_read[i]-start_times[0] <<" "<<end_times_read[i]-start_times[0]<<"\n";
