...
int main(int argc, char* argv[]) {
    ...
    sw.restart();
    _gelu.BackwardFineGrained(bsz_seq, nq, &ff2_buf, (_gelu_checkpoint ? &ff2_inp_ptr : &gelu_inp_ptr), &inter_b, &SE);
    sw.stop();
    ...
    sw.restart();
    _attn_prob_dropout.BackwardFineGrained(bsz_heads * sequence_length, nq, &ff2_buf, &SE);
    sw.stop();
