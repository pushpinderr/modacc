...
int main(int argc, char* argv[]) {
   ...
   if(print_info){
    ...
    if (_pre_or_postLayerNorm)
        _layer_output_dropout.BackwardFineGrained(bsz_seq, buf_0, grad_output_ptr, _stream);
    else
        _layer_output_dropout.BackwardFineGrained(bsz_seq, buf_0, buf_1, _stream);
    ...
    _gelu.BackwardFineGrained(bsz_seq, ff2_buf, (_gelu_checkpoint ? ff2_inp_ptr : gelu_inp_ptr), inter_b_ptr, _stream);
    ...
    if (!_pre_or_postLayerNorm)
        ...
        _attn_output_dropout.BackwardFineGrained(bsz_seq, buf_2, buf_0, _stream);
        ...
        if (_attn_prob_dropout.HasDropout()) {
            ...
            _attn_context.BackwardFineGrained(bsz_heads,
                                   buf_2,
                                   v_tf_ptr,
            ...
            _attn_context.BackwardFineGrained(bsz_heads, buf_2, v_tf_ptr, soft_out_ptr, _cublasHandle, buf_3, ff2_buf);
    
        _attn_prob_dropout.BackwardFineGrained(bsz_heads * _seq_length, ff2_buf, _stream);
    
        _softmax.BackwardFineGrained(bsz, ff2_buf, soft_out_ptr, _stream);
    
        _attn_scores.BackwardFineGrained(bsz_heads, ff2_buf, k_tf_ptr, q_tf_ptr, _cublasHandle, buf_2, buf_1);
