...
struct BertGemmAlgos {
    ...
    BertGemmAlgos()
        : m_gemm_qkv_algo(-1), m_gemm_inter_algo(-1), m_gemm_output_algo(-1),
          m_gemm_batch1_algo(-1), m_gemm_batch2_algo(-1) {}
};

template <typename T> class BertTransformerLayer {
  public:
    BertTransformerLayer(int layer_id, int batch_size, int hidden_size,
                         int num_heads, int intermediate_size, int seq_length,
    ...
    void Forward(int bsz, const T *input_ptr, const T *input_mask_ptr,
                 ...
                 T *add_res_ptr, T *ff1_inp_ptr, T *gelu_inp_ptr,
                 T *ff2_inp_ptr);

    void
    Backward(int bsz, const T *grad_output_ptr, const T *input_ptr,
             const T *output_ptr, const T *inp_norm_ptr, const T *q_tf_ptr,
             const T *k_tf_ptr, const T *v_tf_ptr, const T *softmax_output_ptr,
             const T *ctx_bufB_ptr, const T *attn_o_inp_ptr,
             const T *add_res_ptr, const T *ff1_inp_ptr, const T *gelu_inp_ptr,
